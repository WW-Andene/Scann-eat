...
description: >
  ...
  analysis, architecture mapping, structural diagnosis, dependency analysis, module
...
| Code | Section | What It Does | Trigger Phrases |
|------|---------|-------------|-----------------|
| §R1 | **Codebase Analysis** | Deep read of codebase: what exists, why, how it grew | "analyze my codebase", "understand my app" |
| §R2 | **Architecture Mapping** | Maps actual vs intended architecture, dependency graph | "map my architecture", "show dependencies" |
| §R3 | **Structural Diagnosis** | Identifies anti-patterns, oversized components, coupling, responsibility violations | "diagnose structure", "find structural problems" |
...
| Code | Section | What It Does | Trigger Phrases |
|------|---------|-------------|-----------------|
| §T1 | **Oversized Component Decomposition** | Break up a single oversized component/class | "break up this component", "this file is too big" |
...
| **Plan but execute later** | `"plan restructuring"` → §R1–§R6 |
...
> **For Claude**: When this skill activates, follow these steps in order:
> 1. **Check if a prior app-audit exists.** If yes, load findings — skip §R1–§R3 diagnosis and use audit data instead.
> 2. **If no prior audit exists**: Run §R0 (context) → §R1 (analysis) → §R2 (mapping) → §R3 (diagnosis).
> 3. **Always present the restructuring plan (§R6) to the user before executing any code change.** Wait for explicit user approval before proceeding.
> 4. **Execute one operation at a time.** Each operation = one commit. Verify after each.
> 5. **Keep restructuring commits structure-only.** Separate functional changes (bug fixes, new features) into their own commits outside this workflow.
>
> **For the User**: Say "restructure my app" and Claude will guide you through analysis, planning, and incremental execution. You approve every major decision. Claude changes nothing without your permission.
...
"Break up [specific file/component]"
  → §T1 (Oversized Component Decomposition)
  Claude: targeted operation. Verify before and after.
...
- **§R1 (Codebase Analysis) is the most important analysis phase.** Always run it in full restructuring. It prevents restructuring that accidentally breaks the app's working logic because Claude did not understand why the code was organized that way.
- **§R5 (Target Architecture) requires user approval.** Always present the target architecture and wait for the user to say yes before executing any change.
- **§R6 (Migration Plan) is the contract.** Every operation is listed. Every dependency is tracked. The user reviews this before execution begins.
- **For apps > 5,000 lines:** always confirm with the user after §R3 before continuing to planning.
- **When scope is unclear:** ask the user. Use `AskUserQuestion` with options.
- **Always keep restructuring and feature work in separate commits.** This is a strict rule with no exceptions.
...
TodoWrite([
  ...
  { content: "§R1 — Codebase Analysis", status: "pending" },
...
**Rules:**
- Always keep restructuring commits separate from functional changes
- Each commit must leave the app in a buildable, runnable state
- If a commit breaks the build, revert immediately (§R14)
- Each commit message explains WHAT moved and WHY
...
## GOVERNING RULES — Mandatory Rules for All Restructuring

> **These rules are mandatory.** Every decision, every operation, every commit must follow these rules. When a rule is violated, immediately execute §R14 Rollback Protocol.

### Rule 1 — ALWAYS UNDERSTAND BEFORE CHANGING

**Read and understand every file that will be affected before making any change.** This means:
- Read every file that will be affected by the restructuring operation
- Understand WHY the code is organized the way it is (§R1 Analysis)
- Find hidden dependencies that are invisible in import statements (event listeners, global state, dynamic imports, string-based lookups, reflection)
- If something is unclear, ASK THE USER before touching it

**Compliance check:** Before every file move, Claude can name every file that imports from it, every dynamic reference to it, and every config that points to it.

### Rule 2 — ALWAYS USE ONE OPERATION PER COMMIT

**Every restructuring change is atomic.** One rename = one commit. One extraction = one commit. One move = one commit.

- Each commit transitions the codebase from one valid state to another valid state
- The app MUST build and run after every single commit
- If a commit breaks something, revert it — then diagnose the issue separately
- Always keep restructuring in its own commits, separate from functional changes

**Compliance check:** Every commit diff contains exactly one structural change and zero behavior changes.

### Rule 3 — ALWAYS VERIFY AFTER EVERY OPERATION

**After every restructuring operation, verify all five of these:**
1. The app builds with zero errors
...
**Compliance check:** Claude runs build and test commands after every operation and reports the results before proceeding.

### Rule 4 — ALWAYS PRESERVE BEFORE RESTRUCTURING
...
- Characterization tests for behavior being restructured
- Feature Preservation Ledger tracking what works and must continue working

**Compliance check:** A rollback path exists for every operation before it begins.

### Rule 5 — ALWAYS GET USER APPROVAL FOR THE PLAN

**Claude proposes. The user decides.** Specifically:
- The Target Architecture (§R5) must be presented and approved before execution begins
...
- If Claude discovers mid-execution that the plan needs to change, Claude must STOP and ask the user before continuing

**Compliance check:** The user has said "yes", "approve", "go ahead", or equivalent before any §R7+ execution begins.

### Rule 6 — ALWAYS DEFINE THE TARGET BEFORE MOVING CODE

**The Target Architecture (§R5) must be concrete before any code moves.** This means:
- Every module is named
- Every boundary is defined
- Every convention is documented
- Every file has a planned location

If you cannot describe exactly where something should go, you are not ready to move it. Go back to §R5 and define the target.

**Compliance check:** Every file move in the plan has a specific "from" path and "to" path in the §R5.5 File Migration Table.

### Rule 7 — ALWAYS COLOCATE RELATED CODE

**Things that change together live together.** When deciding where code belongs:
- A component's styles, tests, types, and helpers live next to it — in the same directory
- Feature-specific utilities live in the feature directory
- Only genuinely shared code (used by 3+ features) goes in `shared/`
- When uncertain, keep it colocated — it can always be extracted later

**Compliance check:** Every utility, type, and helper file lives in the directory of the feature that uses it, unless 3+ features import it.

### Rule 8 — ALWAYS USE DESCRIPTIVE NAMES

**Names must communicate structure.** Every file, folder, component, function, and variable name tells the reader:
...
**Compliance check:** Every new file created during restructuring has a name that includes a domain qualifier. Generic names like `helpers.ts`, `utils.ts`, or `misc.ts` always include a domain prefix (e.g., `date-utils.ts`, `auth-helpers.ts`).

### Rule 9 — ALWAYS PRESERVE WORKING PATTERNS

**Restructuring is targeted and precise.** Specifically:
- Preserve architectural systems that work (design token systems, established conventions, working patterns)
- Make targeted changes — one thing at a time
- If the user's existing patterns work but are inconsistent, UNIFY them toward the best existing pattern — use a pattern that already exists in the codebase, not a new invented one
- When multiple conventions exist, the user chooses which one to adopt

**Compliance check:** Every convention chosen in §R5.4 has a "Winner source" that points to an existing file in the codebase.

### Rule 10 — ALWAYS KNOW WHEN TO STOP

**Restructuring has diminishing returns.** Know when to stop:
- Stable code (unchanged in months) that works = leave it alone
- Local debt with zero contagion = leave it alone
- Premature abstraction (extracting before patterns emerge) = leave it alone
- 80% improvement in structure is usually better than 100% that takes 3× longer
- If the user says "that's enough," stop immediately

**Compliance check:** The §R4 Priority Queue has a "DO NOT TOUCH" tier for stable, working, rarely-changed code.

---

### PRIORITY ORDER WHEN RULES CONFLICT

When two rules point to different actions, follow this priority order (highest first):

1. **Rule 5** (User approval) — The user's explicit decision always wins
2. **Rule 4** (Preserve before restructuring) — Safety comes before speed
3. **Rule 3** (Verify after every operation) — Verification comes before progress
4. **Rule 1** (Understand before changing) — Understanding comes before action
5. **Rule 2** (One operation per commit) — Atomicity comes before convenience
6. **Rule 9** (Preserve working patterns) — Working code comes before ideal code
7. **Rule 10** (Know when to stop) — Diminishing returns come before completeness
8. **Rule 7** (Colocate related code) — Colocation comes before organization aesthetics
9. **Rule 6** (Define target before moving) — Planning comes before execution
10. **Rule 8** (Descriptive names) — Naming comes last

---

## §R0 — APP CONTEXT BLOCK (mandatory)

> **Claude execution note**: Extract as much as possible from the codebase automatically before asking the user anything. Only ask for information that cannot be determined from code. Use `Agent` (Explore) and `Bash` (`tree`, `find`, `wc`) to gather data.
...
### R0.2 — User-Provided Context (ask the user for what cannot be extracted from code)
...
Restructuring Intent:
  ...
  Off-limits:         # what must remain unchanged (sacred features, external constraints)
...
> **Claude execution note**: Build this by walking through the app. Every named feature gets an entry. This is the CONTRACT — every WORKING feature must remain working after restructuring. Verify this ledger after every execution phase.
...
> **Claude execution note**: This is the UNDERSTANDING phase. Read deeply. Identify patterns and reasons before making judgments. Use `Agent` (Explore) for parallel reading. Use `Grep` to find patterns. Use `Bash` for quantitative analysis. Present findings to the user as a narrative, not a list.

### R1.1 — Growth Analysis
...
Complexity Anchors (high complexity + low change frequency):
  ...
  # These are usually OK to leave alone (Rule 10)
...
> **Claude execution note**: These are the dependencies that DO NOT show up in import statements. They are the #1 cause of restructuring breakage. Be thorough here. Search for every type listed below.
...
> **Claude execution note**: This phase produces the structural picture. Use `Bash` to generate import graphs. Use `Grep` to find all import/require statements. Describe the architecture textually.

### R2.1 — Actual Architecture Model

Map the current module structure — describe how it actually works, which may differ from how it was intended to work:
...
Dependency Analysis:
  ...
  Layer violations:
    ...
    - {file A} imports from {file B} but should not because {reason}
...
> **Claude execution note**: Ask the user what the architecture is SUPPOSED to be. If they have no clear picture, infer from folder names, README, and conventions.
...
Reflexion Model:
  ...
  Divergences (code dependencies that violate the intended model):
    - {file} in {module A} imports from {module B} — this was not intended
  ...
  Absences (intended dependencies with no code equivalent):
    - {module A} should depend on {module B} but does not
...
> **Claude execution note**: This is the judgment phase. Using data from §R1 and §R2, identify specific structural problems. Each problem gets a severity, category, and recommended resolution strategy. Present grouped by category. Every finding must point to specific files, specific dependencies, specific violations. Do not present generic advice.
...
### R3.2 — Oversized Component / Class Detection

> **Claude execution note**: Use these exact thresholds. If a file exceeds ANY of these, it is a candidate for decomposition.
...
> **Claude execution note**: Not all debt is worth fixing. This phase separates "must fix" from "can leave alone." Use the three-axis model: Impact × Contagion × Fix Cost.
...
Debt Item: {finding ID}
  Type:       LOCAL / MACGYVER / FOUNDATIONAL / DATA
    ...
    # MACGYVER — two systems connected at interface in a fragile way
    # FOUNDATIONAL — deep assumptions built into the core
...
Priority Queue:
  ...
  Tier 4 — Defer (leave alone for now):
    1. {finding ID}: {reason to defer}
    2. ...

  DO NOT TOUCH (Rule 10 — stable, working, rarely changed):
...
> **Claude execution note**: This is the DESIGN phase. Produce a concrete, specific, file-level blueprint of the restructured app. Every file must have a target location. Every module must have defined boundaries. Every convention must be documented. **Always present this to the user for approval before any execution.**

### R5.1 — Architecture Style Decision

> If the user has not specified a style, recommend based on app characteristics:
...
Module: {name}
  ...
  Forbidden deps: # what it must NOT import from (enforces boundaries)
...
> **Claude execution note**: Derive conventions from the BEST existing patterns in the codebase (Rule 9). Use a pattern that already exists in the codebase. If the codebase has multiple conventions, present the options to the user and let them choose.
...
> **Claude execution note**: This is the most important artifact of the planning phase. Every file that moves gets an entry. This table IS the migration plan's source of truth.
...
3. **Extract before move** — decompose oversized components before relocating pieces
...
Risk Assessment:
  Total operations:   #
  HIGH risk ops:      # (oversized component decomposition, state migration, route changes)
  ...
  Abort conditions:
    ...
    - A WORKING feature breaks and cannot be immediately fixed
    - The migration plan proves fundamentally wrong (target architecture does not work)
    - User says stop
```
...
> **Claude execution note**: This phase creates the safety net. Execute every step. These safeguards are what make live restructuring safe.
...
# - Do NOT fix them as part of restructuring (Rule 2 — keep restructuring separate)
# - Note them in the Feature Preservation Ledger as "KNOWN FAILING"
```

### R7.4 — Characterization Tests (if coverage is low)

> **Claude execution note**: If test coverage is below 50% for files being restructured, write characterization tests for critical paths. These document CURRENT behavior (correct or incorrect) so we can detect if restructuring changes it.
...
### R8.2 — Oversized Component Decomposition (when encountered in plan)

> Full protocol at §T1. Summary here for inline reference:

```
1. Read the entire oversized component
...
4. Present decomposition plan to user:
   "I'm going to split {Component} into:
...
> **Claude execution note**: Update all import paths from old locations to new locations. Work through consumers systematically. This is the phase where re-exports are consumed and eventually removed.
...
6. Keep the re-export file for now — removal happens in §R12
```
...
**Verification gate for §R9**: All imports updated. Build passes. Tests pass. Only remaining references to old paths are intentional re-exports to be cleaned up in §R12.
...
Inversion: {high-level module} depends on {low-level module}
  ...
  Verify:     {file A} does not import from {file B} directly
...
State Issue: {description}
  ...
  Steps:
    1. {specific step}
    2. {specific step}
  Verify:     state changes in one feature do not trigger renders in unrelated features
...
```
1. LIST all files that do not match the naming convention (§R5.4)
...
1. For each re-export file created during restructuring:
   ...
   b. If consumers still exist, update them first (go back to §R9)
...
Metrics Comparison:
  ...
  | Oversized components | {X} | {Y} | {diff} | 0? |
...
□ The app does EXACTLY what it did before, with better code structure
```
...
|----|------|-----------|--------|-----------|
| AP-01 | **Big Ball of Mud** | No discernible module boundaries, everything imports everything | Cannot reason about, change, or test in isolation | Full restructuring — define modules, extract features, enforce boundaries |
| AP-02 | **Oversized Component/Class** | LOC > 300, exports > 8, responsibilities > 2, imports > 10 | Hard to understand, test, modify; attracts more code over time | §T1 — Decompose into focused components |
| AP-03 | **Circular Dependencies** | A imports B imports A (direct or transitive) | Cannot isolate modules, bundle size inflation, confusing data flow | §R20 — Interface extraction, event bus, shared abstraction |
...
| AP-19 | **Monolithic State** | Single global store holding all application state | Re-renders everywhere, hard to reason about, hard to test | §T2 — Classify and split by scope: local, feature, server, global |
...
| AP-24 | **Spaghetti Navigation** | Routes/navigation with no clear hierarchy, random deep links | Users get lost, developers cannot predict flow, hard to add screens | Restructure navigation into clear hierarchical graph (§T3) |
...
```bash
# Oversized components (files over 300 LOC)
...
# Dead exports (exported but not imported elsewhere)
...
Rules:
  ...
  - Push 'use client' DOWN to leaf components, keep parents as server components
...
Migration steps:
  ...
  4. ViewModels depend on Repository interfaces (use abstractions, not implementations)
...
Rules:
  ...
  - External navigation goes to the nested graph's START destination only, not to internal destinations
...
Structural Metrics:
  ...
  Oversized components (>300 LOC): # target: 0
...
```
Problem: Two modules are so tightly coupled they cannot be separated
Solution:
...
```
□ The oversized component is identified (LOC > 300 or responsibilities > 2)
□ The component's feature works correctly (we are restructuring, not fixing behavior)
...
STEP 1 — READ AND UNDERSTAND
  Read the entire component. Identify:
...
STEP 3 — NAME EACH CLUSTER
  ...
  Names should NOT be generic: Section1, TopPart, ContentArea
...
STEP 5 — PRESENT PLAN TO USER
  "I'm going to decompose {Component} (currently {LOC} lines) into:
...
STEP 6 — EXTRACT ONE CLUSTER AT A TIME
  ...
  e. BUILD + TEST
  f. COMMIT: "refactor: extract {ClusterName} from {Component}"
...
> **Targeted operation.** Use when navigation is chaotic, deep-linked incorrectly, or does not match user mental model.
...
STEP 4 — ENFORCE DIRECTION
  shared/ must only be imported by features/, and must not import from features/.
...
**Every change must be verifiable and reversible.** If you cannot verify it (no build, no tests, no manual check), do not make it. If you cannot reverse it (no git, no re-exports, no rollback plan), do not make it.
...
1. A WORKING feature breaks and the fix is not immediately obvious
